...
from TehnoMarket.models import Product, Category, Comment, CustomUser, Role, PaymentMethod, Cart, CartItem, Order, OrderItem
from TehnoMarket.forms import ContactForm, LoginForm, RegisterForm, PersonalInfoForm, AddProductForm, AddCommentForm, SelectPaymentMethodForm
    ...
    if not request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                context = {'error': 'Погрешно корисничко име или лозинка!', 'form': login_form}
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if User.objects.filter(username=username).exists():
                    ...
                    return render(request, "registration.html", {'form': form, 'roles': roles, 'username_taken': True})
            ...
            form = RegisterForm()
        return render(request, "registration.html", {'form': form, 'roles': roles, 'username_taken': False})
    else:
        return redirect('/')
...
def SuccessfullyAddedProduct(request):
    if request.user.customuser.role.role != 'Продавач':
        raise Http404
...
def PaymentSuccessful(request):
    if request.user.customuser.role.role != 'Купувач':
        raise Http404
...
def SelectPaymentMethod(request):
    ...
    payment_methods = PaymentMethod.objects.filter(buyer=request.user)  # Filter payment methods by the current user
...
def increase_quantity(request, item_id):
    ...
    if item.cart.user != request.user and request.user.customuser.role.role != 'Купувач':
        raise Http404
...
def my_products(request):
    ...
    if not custom_user.role.role == 'Продавач':
        raise Http404
...
def add_to_cart_view(request, product_id):
    ...
    if user.customuser.role.role != 'Купувач':
        raise Http404

    if product.stock < 1:
        messages.error(request, "Нема достапна залиха на производот.")
        return redirect('products')
...
def save_order(request):
    if request.method == 'POST':
        ...
        for cart_item in cart_items:
            if cart_item.product.stock < cart_item.quantity:
                messages.error(request, "Количината ја надминува достапната залиха за еден или повеќе производи.")
                order.delete()
                return redirect('view_cart')
...
def update_quantity(request, item_id):
    ...
    user = request.user
    if user.customuser.role.role != 'Купувач' and not user.is_superuser and user != cart_item.product.seller:
        raise Http404
...
def product_sent_to_admin(request):
    ...
    if user.customuser.role.role != 'Продавач':
        raise Http404
...
def edit_product_image(request, id):
    ...
    if not user.is_superuser:
        ...
        if role is None or role.role == "Купувач":
            raise Http404
